...
class project_control extends phpok_control
{
	...
	function index_f()
	{
		...
		$pid = $tmp['id'];
		$rs = $this->model('data')->project(array('pid'=>$pid,'project_ext'=>true));
		if(!$rs) error("项目不存在或操作异常或项目未启用","","error");
		$this->phpok_seo($rs);
		...
		$tpl = $rs["tpl_index"] ? $rs["tpl_index"] : ($rs["tpl_content"] ? $rs["tpl_content"] : $rs["tpl_list"]);
		if(!$tpl && $rs["parent_id"] && $parent_rs)
		{
			//如果父级栏目有设置了模板页
			$tpl = $parent_rs["tpl_content"] ? $parent_rs["tpl_content"] : $parent_rs["tpl_list"];
			if($parent_rs['tpl_index']) $tpl = $parent_rs['tpl_index'];

			//到这一步，如果还没有设置页面
			if(!$tpl) $tpl = $parent_rs["identifier"]."_page";
		}
