...
class order_control extends phpok_control
{
	...
	function create_f()
	{
		if(!$_SESSION['order_spam'] || !$this->get('order_spam') || $_SESSION['order_spam'] != $this->get('order_spam'))
		{
			$this->json('提交表单操作非法，请检查表单提交来源');
		}
		//判断购物车里是否有产品信息
		$rslist = $this->model('cart')->get_all($this->cart_id);
		if(!$rslist)
		{
			$this->json("您的购物车里没有任何产品！");
		}
		$totalprice = 0;
		$qty = 0;
		...
		$array['status'] = 'NOTCHECKED';
		$array['pay_id'] = $payment;
		$array['pay_price'] = $totalprice;
		$array['pay_currency'] = $this->site['currency_id'];
		$array['pay_status'] = 'NOTPAID';
		$array['passwd'] = md5(str_rand(10));
		$oid = $this->model('order')->save($array);
		if(!$oid) $this->json('订单创建失败，请检查<pre>'.print_r($array,true));
		//存储订单产品信息
		foreach($rslist AS $key=>$value)
		{
