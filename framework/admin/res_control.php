...
class res_control extends phpok_control
{
	...
	function cut_f()
	{
		...
		{
			error_open("此操作仅支持：JPG，GIF和PNG图片裁剪，其他图片请转码");
		}
		$this->assign("rs",$rs);
		$this->assign("id",$id);
		...
		if($gd)
		{
			$list = $this->model('gd')->get_all("identifier");
			if($list[$type])
			{
				$thumb_rs = $list[$type];
			}
			...
			{
				error_open("GD类型不存在，请检查");
			}
			$form_url = admin_url("res","cut_save") ."&id=".$id."&type=".$type."&gd=1";
		}
		else
		{
			$form_url = admin_url("res","cut_save") ."&id=".$id."&type=ico&gd=0";
			$thumb_rs = array("width"=>"100","height"=>"100","cut_type"=>1);
		}
		$this->assign("gd_type",$type);
		$this->assign("form_url",$form_url);
		if($thumb_rs["width"] && $thumb_rs["height"])
		{
			if($rs["attr"]["width"] < $thumb_rs["width"] || $rs["attr"]["height"] < $thumb_rs["height"])
			{
				error_open("原图宽高不符合裁剪要求，请上传宽高不小于：".$thumb_rs["width"]." &#215; ".$thumb_rs["height"]."");
			}
			//计算预览的宽高
			$preview_width = $thumb_rs["width"] > 300 ? 300 : $thumb_rs["width"];
			$preview_height = ($preview_width * $thumb_rs["height"])/$thumb_rs["width"];
			$this->assign("preview_width",$preview_width);
			$this->assign("preview_height",$preview_height);
		}
		else
		{
			$preview_width = 300;
			$preview_height = ($rs["attr"]["height"] * $preview_width) / $rs["attr"]["width"];
			$this->assign("preview_width",$preview_width);
			$this->assign("preview_height",$preview_height);
		}
		$file_width = $rs["attr"]["width"] > 500 ? 500 : $rs["attr"]["width"];
		$file_height = ( $rs["attr"]["height"] * $file_width ) / $rs["attr"]["width"];
		$this->assign("file_width",$file_width);
		$this->assign("file_height",$file_height);
		$this->assign("gd",$thumb_rs);
		$this->view("res_cut");
	}
	...
	function update_pl_f()
	{
		...
		if($id == 'all')
		{
			...
			if(!$reslist)
			{
				error_open("附件信息更新完毕，共更新数量：<span class='red'>".$pageid."</span>","ok",'<input type="button" value="关闭" onclick="$.dialog.close()" />');
			}
			$rs = current($reslist);
		...
		else
		{
			...
			if(!$list[$pageid])
			{
				error_open("附件信息更新完毕，共更新数量：<span class='red'>".count($list)."</span>","ok",'<input type="button" value="关闭" onclick="$.dialog.close()" />');
			}
