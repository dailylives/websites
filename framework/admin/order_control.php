...
class order_control extends phpok_control
{
	...
	function index_f()
	{
		if(!$this->popedom["list"]) error("你没有查看权限",$this->url('index'));
	...
	function save_f()
	{
		...
		if(!$id)
		{
			if(!$this->popedom['add']) $this->json('您没有创建订单权限');
			$this->add_save();
		}
		else
		{
	...
	function modify_save($id)
	{
		...
		$main['pay_currency'] = $this->get("pay_currency",'int');
		$main['pay_status'] = $this->get('pay_status');
		if($main['pay_status'] == 'PAID')
		{
			$pay_date = $this->get('pay_date');
			if(!$pay_date) $pay_date = date("Y-m-d H:i",$this->time);
	...
	function add_save()
	{
		...
		//存储订单操作
		$main['price'] = $this->get('price','float');
		if(!$main['price']) $main['price'] = $total_price;
		$main['currency_id'] = $this->get('currency_id','int');
		$main['user_id'] = $this->get('user_id','int');
		$main['pay_id'] = $this->get('pay_id','int');
		$main['pay_price'] = $this->get('pay_price','float');
		if(!$main['pay_price']) $main['pay_price'] = $total_price;
		$main['pay_currency'] = $this->get("pay_currency",'int');
		$main['addtime'] = $this->time;
		$main['qty'] = $total_qty;
		$main['status'] = 'NOTCHECKED';
		$main['pay_status'] = $this->get('pay_status');
		if($main['pay_status'] == 'PAID')
		{
			$pay_date = $this->get('pay_date');
