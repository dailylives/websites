...
function _domain_check(domain,isadd,domain_id)
{
	...
	//检测此域名是否被使用
	var url = get_url("all","domain_check") + "&domain="+rawurlencode(domain);
...
function domain_add()
{
	...
	var url = get_url("all","domain_save")+"&domain="+rawurlencode(domain);
	var rs = json_ajax(url);
	if(rs.status == "ok")
	{
		alert("域名添加成功！");
...
function g_ext_check()
{
	...
	//检测字串是否被使用了
	var url = get_url("all","all_check")+"&identifier="+rawurlencode(identifier);
	if(id)
	{
		url +="&id="+id;
	}
